...
module.exports = {
    'play': (msg, command) => {
        ...
        if (msg.guild.me.voiceChannel) return msg.channel.send(`Playing at ${msg.guild.me.voiceChannel}`);
        const raw = stringhandler.cutTextHead('play ', command);
        if (!raw) return msg.channel.send("There are no factor.");
        let url = raw;
                ...
                try {
                    function show(msg, count) {
                        let embed = new Discord.RichEmbed()
                            .setAuthor(`${msg.author.tag}`, msg.author.displayAvatarURL)
                            .setColor(`${config.color}`)
                            .setTitle(url + ' Search Result')
                            .setFooter(count + " Page. prev | next");
    ...
    /*'sc': (msg, command) => {
        ...
        if (!raw1) return msg.channel.send("There are no factor.");
            ...
            request("http://api.soundcloud.com/resolve.json?url=" + raw1 + "&client_id=71dfa98f05fa01cb3ded3265b9672aaf", function (error, response, body) {
                if (error) msg.reply(error);
                    ...
                    let embed = new Discord.RichEmbed()
                        ...
                        .addField("Information", " by **" + body.user.username + "** *(" + Math.floor(body.duration / 1000) + " 초)* ")
                        .setFooter("#" + body.id);
                    msg.channel.send(embed);
                    msg.member.voiceChannel.join().then(connection => {
